...
EMmaxIter = 4;      % EMmaxIter > 1: after finding one strain design, 
...
product = 'EX_succ(e)';
prodind = find(strncmp(product,model.rxns,length(product)));
...
gsmodel=struct;
gsmodel.('strain0') = model;
strainSummary = struct;
for i=1:length(strains)
    strainName = sprintf('strain%d',i);
    strainSummary.(strainName) = printEMILIOmods(model, strains{1,i}, 0);
    gsmodel.(strainName)=gsmodel.('strain0');
    gsmodel.(strainName).vl=strains{1,s}.vl;
...
hold on
models = struct;
models.wt = changeRxnBounds(model,{'EX_o2(e)'},-20,'l');
productionEnvelope(models.wt,{},'black', 'EX_succ(e)','Ec_biomass_iAF1260_core_59p81M');
...
%    models.(modelName).ub = models.(modelName).vu;
    %productionEnvelope(models.(modelName),{},cc(s,:), 'EX_succ(e)','Ec_biomass_iAF1260_core_59p81M');
%   productionEnvelope(models.(modelName),{},'k','EX_succ(e)','Ec_biomass_iAF1260_core_59p81M');
%end
...
legend(lineLegend)
title('Succinate EMILIO production envelope')
xlabel('Growth rate [1/hr]')
ylabel('Succinate accumulation [mmol/gDW/hr]')
